...
export class ConfigService {
  ...
  private loadConfiguration() {
    try {
      ...
      if (!fs.existsSync(taxonomyPath)) throw new Error(`Taxonomy file missing at ${taxonomyPath}`);
      const taxonomyRaw = fs.readFileSync(taxonomyPath, 'utf-8');
      this.taxonomy = JSON.parse(taxonomyRaw); // 💥 THROWS if JSON is bad

      if (!Array.isArray(this.taxonomy)) throw new Error("Taxonomy must be a JSON Array");

      // 3. Load System Prompt (Text)
      if (!fs.existsSync(promptPath)) throw new Error(`Prompt file missing at ${promptPath}`);
      this.systemPrompt = fs.readFileSync(promptPath, 'utf-8');

      this.logger.log(`✅ Configuration loaded. ${this.taxonomy.length} sectors loaded.`);
  ...
  getCombinedPrompt() {
    ...
    ${JSON.stringify(this.taxonomy)}

    OUTPUT FORMAT:
    { "secteur": "String", "confidence": "High/Medium/Low", "reasoning": "String" }
    `;
  }
}
